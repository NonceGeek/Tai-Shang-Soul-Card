...
export default function index() {
  ...
  const goYoutube = () => {
    window.open(`https://www.youtube.com/watch?v=HVT74l21lB0`);
  };
  return (
    <div className="header w-main py-4 flex justify-between items-center">
      <NavLink className="left flex items-center cursor-pointer" to="/">
        <img className="logo w-12 h-12" src={LogoImg} alt="logo" />
        <span className="site-name pl-3 text-[32px] text-white font-Audiowide">
          SoulCard
        </span>
      </NavLink>
      <div className="right flex items-center gap-x-8 text-[20px]">
        <div
          className="!text-white font-IBMPlexMono cursor-pointer flex items-center"
          onClick={goYoutube}
        >
        ...
        {!address && (
          <>
            <Button
              ...
              onClick={() => connect()}
            />
            <Button colorStyle="green" buttonText="Sign up" withSpace={false} />
          </>
        )}
        {address && (
            ...
            <div className="network h-8 rounded bg-[#333333] flex items-center relative cursor-pointer">
              ...
              <div className="hidden peer-hover:block hover:block absolute z-10 top-8 w-[176px] pt-3">
                <div className="flex flex-col items-start rounded px-4 py-2 pb-4 bg-[#333333] text-white">
                  ...
                  {availableChains.map((theChain) => (
                    <div
                      className={classNames(
                        'my-0.5 flex items-center px-4 py-2 gap-x-2',
                        theChain.id === chain?.id
                          ? 'w-full bg-[#282828] rounded'
                          : '',
                      )}
                      key={theChain.id}
