...
const Card = (props) => {
  ...
  return (
    <div className="card-container-dao text-white">
      ...
      <div className="flex">
        <img src={address} alt="" />
        <span className="ml-10 ft-s-14 bg-grey text-gray addr word-break">
          {addr}
        </span>
      </div>
      <div className="inside text-ibm mt-8">
        <div className="basic-info general-border">
          <img
            className="w-[128px]"
            src={cardData.avatar ? cardData.avatar : dao_avatar}
            alt=""
          />
        </div>
      ...
      <div className="awsome-things general-border mt-8">
        ...
        {cardData.awesome_things.map((item) => {
          return (
            <div
              className="border-t p-8 fw-700 text-ibm ft-s-12 flex"
              key={item.project}
            >
      ...
      <div className="awsome-things general-border mt-8">
        ...
        <div className="dao-list">
          {coreMember.map((item, index) => {
            return (
              <div className="flex pt-4 pl-8 mb-15" key={index}>
                <div>
                  <img src={require(`./mock/${item.avatar}.png`)} alt="" />
                </div>
                <div className="ml-10">
                  <div className="dao-name ft-s-14 ft-700 mb-5 line-height-one text-ibm-bold word-break">
                    {item.name}
                  </div>
                  {item.is_core_member ? (
      ...
      <div className="awsome-things general-border mt-8">
        ...
        <div className="partner">
          {cardData.partner.map((item, index) => {
            return (
              <div className="flex border-t p-8 dao-ele" key={index}>
                <img src={require(`./mock/${item.avatar}.png`)} alt="" />
